		...
		<ul>
			...
			<li>
				<span class="viewAllListings">
					<a href="{page_path id='users_listings'}{$user.id}/{if $user.DealershipName.exists}{$user.DealershipName|regex_replace:"/[\\/\\\:*?\"<>|%#$\s]/":"-"}-{/if}{$user.FirstName|regex_replace:"/[\\/\\\:*?\"<>|%#$\s]/":"-"}-{$user.LastName|regex_replace:"/[\\/\\\:*?\"<>|%#$\s]/":"-"}.html">[[View All Listings]]</a>
				</span>
			</li>
		</ul>
	</div>
