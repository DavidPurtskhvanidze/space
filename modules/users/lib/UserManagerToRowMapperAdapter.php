<?php
/**
 *
 *    Module: users v.7.5.0-1, (c) WorksForWeb 2005 - 2016
 *
 *    Package: users-7.5.0-1
 *    Tag: tags/7.5.0-1@19887, 2016-06-17 13:25:03
 *
 *    This file is part of the 'users' module of the WorksForWeb
 *    software. The entire content is protected by the applicable national
 *    and international copyright legislation.
 *
 */


namespace modules\users\lib;

class UserManagerToRowMapperAdapter
{
	private $userManager;
	private $extraPropertySetters;
	
	public function __construct($userManager)
	{
		$this->userManager = $userManager;
		$this->extraPropertySetters = array();
	}
	
	public function addUserExtraPropertySetter($extraPropertySetter)
	{
		$this->extraPropertySetters[] = $extraPropertySetter;
	}
	
	public function mapRowToObject($row)
	{
		$user = $this->userManager->createUser($row, $row['user_group_sid']);
		foreach ($this->extraPropertySetters as $extraPropertySetter)
		{
			$extraPropertySetter->setUser($user);
			$extraPropertySetter->perform();
		}
		
		return $user;
	}
}
