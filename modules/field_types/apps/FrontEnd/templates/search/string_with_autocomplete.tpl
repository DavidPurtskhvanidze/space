...
{/if}
<input type="text" id="{$element_id}" name="{$id}[like]" class="form-control stringWithAutocomplete {$id}"{if $maxlength > 0} maxlength="{$maxlength}"{/if} value="{$value.like|escape}" placeholder="{$placeholder}" />
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#{$element_id}").on('autocompleteclose',function(){
