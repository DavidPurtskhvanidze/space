{foreach from=$tree_values.$parent item=node}
    ...
    <option value="{$node.sid|escape}" title="{$node.path}" {if $node.sid eq $selected}selected="selected"{/if} >
        ...
        {$smarty.capture.node_caption|indent:$node.level:" &nbsp; &nbsp;"}
    </option>

    {if isset($tree_values.$node_sid)}
        {include file="field_types^search/category_tree_option_no_digits.tpl" tree_values=$tree_values parent=$node.sid selected=$selected}
    {/if}
{/foreach}
