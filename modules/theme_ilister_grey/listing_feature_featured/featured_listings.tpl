            ...
            <div class="item">
                ...
                <div class="fieldValue fieldValuePictures">
                    {if $listing.pictures.numberOfItems > 0}
                        ...
                        {else}
                        <a href="{$listingUrl}"><img src="{url file='main^no_image_available_small.png'}"
                                                     alt="[[No photos:raw]]" class="noImageAvailable"/></a>
                    {/if}
                </div>
                <div class="details">
                    {if $listing.Price.exists && !$listing.Price.isEmpty}
                        <span class="fieldValue fieldValuePrice money"><a
                                href="{$listingUrl}">{$GLOBALS.custom_settings.listing_currency}
                            [[$listing.Price]]</a></span>
                    {/if}
                    {if $listing.Rent.exists && !$listing.Rent.isEmpty}
                        <span class="fieldValue fieldValueRent money"><a
                                href="{$listingUrl}">{$GLOBALS.custom_settings.listing_currency}
                            [[$listing.Rent]]</a></span>
                    {/if}
                    <a href="{$listingUrl}">
