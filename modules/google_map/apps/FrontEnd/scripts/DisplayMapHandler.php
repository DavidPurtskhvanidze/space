...
namespace modules\google_map\apps\FrontEnd\scripts;

class DisplayMapHandler extends \apps\FrontEnd\ContentHandlerBase
{
	...
	public function respond()
	{
		...
		if (!is_null($geoCode))
		{
			$templateProcessor->assign('latitude', floatval_ignore_locale($geoCode['latitude']));
			$templateProcessor->assign('longitude', floatval_ignore_locale($geoCode['longitude']));
			$templateProcessor->display('google_map.tpl');
		}
		elseif (!is_null($defaultLatitude) && !is_null($defaultLongitude))
		{
			$templateProcessor->assign('latitude', floatval_ignore_locale($defaultLatitude));
			$templateProcessor->assign('longitude', floatval_ignore_locale($defaultLongitude));
			$templateProcessor->display('google_map.tpl');
		}
	}
}
