...
<div class="treeField">
				...
				{foreach $tree_values.$parent as $tree_value}{* for all of the children of the particular parent f.e. for all makes or for all models of particular make*}
					{if $value == $tree_value.sid} {*searching for element by it's SID*}
						<div class="removable"> {*as we find required element we add it to the form*}
							...
							<div class="removeLinkContainer">
								<a href="#" class="removeAction{if $parent==0} parent{/if}" title="Remove"></a>
							</div>
						</div>
					{/if}
				{/foreach}
	...
	<div class="addMoreOptionsPopUp {$id}" title="[[Add Make & Model]]">
		{for $index=0 to $tree_depth-1}
			<select class="searchTreeLevel{$index+1}" name="{$id}[tree][{$index}]">
...
<script>
	$(function () {
		...
		$(".addMoreOptionsPopUp.{$id}").dialog(
				{
					autoOpen: false,
					modal: true,
					buttons: {
						"[[Add]]": function () { //Pop up has one button "Add"

							var newItemsWasAdded = false; //Will be a trigger to update or do not the Refine Search Form
							var parentsCaption = ''; // Will be userd for tree children to obtain its parent info
							var parentsValue = ''; // Will be userd for tree children to obtain its parent info
							for (var i = 1; i <= treeDepth; i++) { //Adding new element to form
								// Geting the info
								var value = $(".addMoreOptionsPopUp.{$id} select.searchTreeLevel" + i).val();
								var caption = $(".addMoreOptionsPopUp.{$id} select.searchTreeLevel" + i + " option:selected").text();
								var parent = "";
								if (value == ""){
									break;
								}
								...
								if (i == 1) { // is a parent
									...
									if ($('[data-parent-caption="'+caption+'"]').size()==0){ //If the element wasn't added before
										formElement = "<div class='removable'>"
												...
												+ "<div class='removeLinkContainer'><a href='#' class='removeAction parent' title='[[Remove]]'></a></div></div>";
		...
		$(".showAddMoreOptionsPopUp.{$id}").click(function(e){
			e.preventDefault();
			$(".addMoreOptionsPopUp.{$id}").dialog("open");
		});
