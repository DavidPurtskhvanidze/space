...
<div class="compareListingsPage">
	...
	{function name=displayComparisonTableRow}
		<tr class="{cycle values='oddrow,evenrow'}">
			<td class="fieldCaption fieldCaption{$field.id}">
	...
	{/function}
		...
		<table border="0" cellpadding="0" cellspacing="0" style="width:{math equation="x*200+200" x=$listings|@count}px"> {* 200px - compared item td width, 200px - caption td width *}
			<tr class="{cycle values='oddrow,evenrow'} controls">
				<td>
					<span>
						<a class="massActionControls clear" href="{page_path module='classifieds' function='clear_comparison'}">[[Remove all]]</a>
					</span>
				</td>
				...
				{foreach from=$listings item=listing}
					<td>
						<span>
							<a class="itemControls remove" href="{page_path module='classifieds' function='remove_from_comparison'}?listing_id={$listing.id}&amp;HTTP_REFERER={$currentPageUrl}">[[Remove]]</a>
						</span>
					</td>
				{/foreach}
			</tr>
