...
use modules\classifieds\lib\SearchListingsHelper;

class SearchResultsHandler extends \apps\FrontEnd\ContentHandlerBase
{
	protected $displayName = 'Search Form';
	protected $moduleName = 'classifieds';
	protected $functionName = 'search_results';
	protected $parameters = ['default_sorting_field', 'default_sorting_order', 'default_listings_per_page', 'results_template', 'advanced_search_form_uri'];
	protected $resultViewTypeTemplate = null;
	protected $resultViewTypeControls = [];
	...
	public function respond()
	{
		...
		try
		{
			...
			$searchListingsHelper = new SearchListingsHelper();
	...
	public function registerResources($templateProcessor)
	{
		...
		$beforeListingSearchExecutedActions = new ExtensionPoint('modules\classifieds\apps\FrontEnd\IBeforeListingSearchExecuted');
		...
		$afterListingSearchExecutedActions = new ExtensionPoint('modules\classifieds\apps\FrontEnd\IAfterListingSearchExecuted');
    ...
    private function getSearch()
    {
		...
		{
			$onNewListingSearchActions = new ExtensionPoint('modules\classifieds\apps\FrontEnd\IOnNewListingSearch');
    ...
    private function getSearchFromSession($searchId)
    {
        ...
        if ($ss)
        {
            ...
            $searchFromURI = $search->getSearchFormUri();
	...
	private function getModelListing()
	{
		...
		{
            $searchListingsHelper = new SearchListingsHelper();
			$this->modelListing = \App()->ListingFactory->getListing([], $searchListingsHelper->getCategorySid());
			...
			$this->modelListing->addFirstActivationDateProperty();
			$onSearchModelListingCreatedActions = new ExtensionPoint('modules\classifieds\apps\FrontEnd\IOnSearchModelListingCreated');
	...
	{
		$this->resultViewTypeControls = [];
	
		$viewTypes = new ExtensionPoint('modules\classifieds\apps\FrontEnd\ISearchResultViewTypeOption');
