<div class="editListValues">
    <div class="breadcrumbs">
		...
		{/foreach}
		<a href="{page_path id='edit_category_field'}?sid={$field_sid}">[[$field_info.caption]]</a> &gt; [[Edit List]]</li>
        </ul>
    </div>
	...
	<a class="btn btn-link" href="{page_path module='classifieds' function='import_list_data'}?field_sid={$field_sid}">[[Import data from file]]</a></li>
    ...
    <form method="post" class="form-horizontal" role="form" action="" name="itemSelectorForm">
        ...
        <div class="row">
            <div class="col-xs-12 usersBlock">
                <table class="items sortable table table-striped table-hover" data-parent-value="{$field_sid}" data-sorting-url="{page_path id='edit_listing_field_edit_list'}">
                    <thead>
                        <tr class="head">
                            <th style="width:80px !important;">
                                <div class="multilevelMenu" style="width:150px !important;">
                                    <ul>
                                        <li>
                                            <div class="slideToggler2"><b class="arrow icon-angle-down"></b>[[Mass Actions]]</div>
                    ...
                    <tbody>
                        {foreach from=$list_items item=list_value key=sid name=items_block}
                            <tr class="editableContainer {cycle values="odd,even"}" data-item-sid="{$sid}" id="{$sid}">
                                ...
                                <td><div class="btn-group"><a href="#"  class="itemControls edit btn btn-xs btn-info" title="Edit" data-rel="tooltip" data-original-title="[[Edit]]"><i class="icon-edit bigger-120"></i></a><a class="itemControls delete btn btn-xs btn-danger" href="?field_sid={$field_sid}&action=delete&item_sid={$sid}" onclick="return confirm('[[Are you sure?:raw]]')" title="[[Delete]]" data-rel="tooltip" data-original-title="[[Delete]]"><i class="icon-trash bigger-120"></i></a></div></td>
                                <td></td>
                                <td class="sort">
    ...
    <div id="errorMessageDialog" class="modal fade bs-example-modal-sm" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
...
<script type="text/javascript">
	...
	var editCaptionUrl = "{page_path id='edit_listing_field_edit_list_item'}";
